...
import com.gamegards.allinonev3.Comman.DialogForgetPassword;
import com.gamegards.allinonev3.Interface.ApiRequest;
import com.gamegards.allinonev3.Interface.Callback;
import com.gamegards.allinonev3.Interface.ClassCallback;
...
import com.gamegards.allinonev3.Utils.ImageTakenSingleTone;
...
//        lnrThirdScreen = findViewById(R.id.lnrThirdScreen);
                ...
                if(isValid())
                {
                    if(signupView.getCurrentView() == lnrFirstScreen)
                    {
                        if (CommonFunctions.isNetworkConnected(context)) {
                            //phoneLogin();
                            // Toast.makeText(context,genderradioButton.getText(),
                            ...
                            if (isSelected) {
                                ...
                                Toast.makeText(context, "Please select Gender first ?", Toast.LENGTH_SHORT).show();
    ...
    private boolean isValid() {
        ...
        if(signupView.getCurrentView() == lnrFirstScreen)
        {
            ...
            else if(!Funtions.checkisStringValid(Funtions.getStringFromEdit(edtPassword)))
            {
                return showToastNReturnFalse("Enter Password");
            }
            else
            if (!Funtions.checkisStringValid(Funtions.getStringFromEdit(edtPhone))) {
...
//            }
        ...
        else if(signupView.getCurrentView() == lnrSeconScreen)
        {
            ...
            else if(!Funtions.checkisStringValid(Funtions.getStringFromEdit(edtdob)))
            {
                return showToastNReturnFalse("Enter Date of birth");
            }
            else if(!Funtions.checkisStringValid(Funtions.getStringFromEdit(edtState)))
            {
                    ...
                    if (code.equalsIgnoreCase("201")) {
                        ...
                        if (jsonObject.has("user")) {
                            ...
                            if (jsonObject.has("message")) {
                                ...
                                Toast.makeText(context, "Wrong mobile number or password", Toast.LENGTH_LONG).show();
                            }
...
//                params.put("gender", value.trim());
                ...
                Funtions.LOGE("LoginScreen","params : "+params);
                return params;
            }
    ...
    private void loginwithPassword() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, Const.login_withpassword,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Funtions.LOGE("LoginScreen",Const.login_withpassword+"\n"+response);
                        progressDialog.dismiss();
                        try {
...
//                params.put("name", edtname.getText().toString());
                ...
                if(!isLoginView)
                {
                    ...
                    if(panBitmap !=null)
                        params.put("pan_card", Funtions.Bitmap_to_base64(context,panBitmap));
    ...
    public void SendOtp(View view) {
        ...
        if(!Funtions.checkStringisValid(Funtions.getStringFromEdit(edtPhoneLogin)))
        {
            Funtions.showToast(context,"Enter Mobile Number.");
            return;
        }
        else if(!Funtions.checkStringisValid(Funtions.getStringFromEdit(edtPasswordLogin)))
        {
            Funtions.showToast(context,"Enter Password.");
            return;
        }
